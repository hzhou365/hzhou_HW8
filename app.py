...
def welcome():
    ...
    return (
        ...
        f"/api/v1.0/tobs<br/>"
        f"/api/v1.0/trip/\"startdate\"<br/>"
        f"/api/v1.0/trip/\"startdate\"/\"enddate\"<br/>"
    )
...
def trip(start,end="2017-08-23"):
    """Return TMIN TAVG TMAX from the dataset"""
    
    exp_temps = calc_temps(start, end)
    min_temp = exp_temps[0][0]
    avg_temp = exp_temps[0][1]
    max_temp = exp_temps[0][2]

    list_of_temps=[]
    list_of_temps.append(min_temp)
    list_of_temps.append(avg_temp)
    list_of_temps.append(max_temp)
